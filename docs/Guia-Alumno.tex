...
Esta guia te explica como entregar tus evaluaciones de programacion usando una app automatica que sube tu codigo a GitHub.
...
Solo tres pasos: clonar el repositorio que ya aceptaste en casa, programar tu evaluacion en IDLE, y subir los archivos. La app entrega automaticamente el enlace que pegas en el AVA.
\end{successbox}
...
\begin{itemize}
  ...
  \item No necesitas instalar git ni gh manualmente (la app lo hace solo)
...
\section{Paso 1\,---\,Descargar la app}

\begin{enumerate}
  \item El profesor te entregara el instalador autocontenido de la app (o el enlace a la seccion \textbf{Releases} del repo).
  \item Descargalo y guardalo en una carpeta facil de encontrar (por ejemplo \texttt{Escritorio\textbackslash{}DUOC}).
  \item La app no requiere instalacion de dependencias manuales: detecta y ofrece instalar \texttt{git} y \texttt{gh} (GitHub CLI) automaticamente si no estan presentes.
\end{enumerate}

\begin{notebox}[Cliente legacy removido]
El cliente viejo basado en PowerShell (\texttt{Subir-Evaluacion.bat} / \texttt{.ps1}, \texttt{Reset-GitHubAuth.ps1}) fue removido del repo. La app de escritorio self-contained (C\#) lo reemplaza.
\end{notebox}
...
\begin{enumerate}
  \item Haz doble-click en el ejecutable de la app.
  ...
  \item Listo. Vuelve a la app y click \textbf{Ya tengo cuenta, iniciar sesion}.
\end{enumerate}
...
La primera vez que abres la app, te ofrecera instalar \texttt{git} y \texttt{gh} (GitHub CLI) usando \texttt{winget}.
...
\begin{enumerate}
  ...
  \item Cuando termine, \textbf{cierra y vuelve a abrir} la app para que detecte las nuevas herramientas.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Si no estas conectado, la app muestra un dialogo con un \textbf{codigo} grande tipo \texttt{XXXX-XXXX}.
  ...
  \item La app detecta automaticamente la autorizacion y continua.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Cierra el navegador y vuelve a la app.
\end{enumerate}
...
\section{Paso 7\,---\,Cerrar la app y guardar el PC}

Si trabajas desde tu PC personal, cierra la app normalmente y apaga. Todo queda configurado.

\textbf{El dia de la evaluacion solo tendras que abrir la app y hacer clone + edit + push.}
...
\section{Paso 1\,---\,Abrir la app}

\begin{enumerate}
  \item Si el PC del aula ya tiene la app: doble-click en el ejecutable.
  \item Si NO lo tiene: descargalo de nuevo del enlace que entrega el profesor.
  \item En PCs del aula puede que tengas que repetir el login (Paso 5 del capitulo anterior).
\end{enumerate}

\section{Paso 2\,---\,Verificar tu curso, seccion y evaluacion}

Mira los dropdowns \emph{\textquotedblleft\,Tu curso\,\textquotedblright}, \emph{\textquotedblleft\,Tu seccion\,\textquotedblright} y \emph{\textquotedblleft\,Tu evaluacion\,\textquotedblright} en el panel izquierdo: deben decir tu curso, seccion y evaluacion correctos. Si no, cambialos.
...
\begin{enumerate}
  \item Vuelve a la app (esta abierta en la barra de tareas).
...
La app te preguntara: \emph{\textquotedblleft\,Ya terminaste la evaluacion\,?\,\textquotedblright}
...
La app desbloquea automaticamente los archivos al iniciar para evitar que esta advertencia se repita.
...
Click \textbf{Si} cuando la app ofrezca iniciar sesion. Sigue las instrucciones del dialogo con el codigo.
...
\begin{enumerate}
  ...
  \item Tienes conexion a internet (la app consulta el backend cada 60 segundos).
\end{enumerate}
...
\begin{lstlisting}
1. Descargar la app (Releases del repo o ZIP del profe)
2. Doble-click en el ejecutable
3. Aceptar instalacion winget (Si)
4. Elegir curso / seccion / evaluacion
...
\begin{lstlisting}
1. Abrir la app (doble-click ejecutable)
